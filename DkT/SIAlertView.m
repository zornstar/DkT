...
- (void)validateLayout
{
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2) {
            CGFloat width = (self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2 - GAP - BUTTON_PADDING * 2) * 0.5;
            UIButton *button = self.buttons[0];
            button.frame = CGRectMake(CONTENT_PADDING_LEFT + BUTTON_PADDING, y, width, BUTTON_HEIGHT); //mz change
            button = self.buttons[1];
            button.frame = CGRectMake(CONTENT_PADDING_LEFT + width + GAP + BUTTON_PADDING, y, width, BUTTON_HEIGHT); //mz change
        } else {
            for (NSUInteger i = 0; i < self.buttons.count; i++) {
                UIButton *button = self.buttons[i];
                button.frame = CGRectMake(CONTENT_PADDING_LEFT + BUTTON_PADDING, y, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2 - BUTTON_PADDING * 2, BUTTON_HEIGHT);
